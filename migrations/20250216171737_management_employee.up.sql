...
create type management.employee_marital_status as enum(
    'Single',
    'Married',
    'Divorced',
    'Widowed',
...
create table
    management.department_employees (
...
create table
    management.emergency_information (
        id uuid primary key default gen_random_uuid (),
...
create trigger create_basic_user_employee_trigger before insert on management.employee for each row
...
create policy "only admin can create department" on management.department for insert
with
    check (
        exists (
            select
...
create policy "only admin can add employee to different departments" on management.department_employees for insert
with
    check (
        exists (
            select
...
create policy "employees under a department can see its information" on management.department for
select
    using (
        exists (
            select
...
create policy "only admin can update department" on management.department for
update using (
    exists (
        select
            1
...
create policy "only admin can transfer employee to a different department" on management.department_employees for
update using (
    exists (
        select
            1
...
-- job information:
alter table management.job_information enable row level security;

create policy "only admin can create job information" on management.job_information for insert
with
    check (
        exists (
            select
...
create policy "any role can see each others job information" on management.job_information for
select
    using (
        exists (
            select
...
create policy "only admin can update job information" on management.job_information for
update using (
    exists (
        select
            1
...
alter table management.emergency_information enable row level security;

create policy "admin can create emergency information to all employee" on management.emergency_information for insert
with
    check (
        exists (
            select
...
create policy "any role can see emergency information" on management.emergency_information for
select
    using (
        exists (
            select
...
create policy "admin or the current_user can update its emergency information" on management.emergency_information for
update using (
    exists (
        select
            1
...
create policy "only admins can delete emergency information" on management.emergency_information for
update using (
    exists (
        select
            1
...
create policy "any employee can request pan" on management.personnel_action for insert
with
    check (
        exists (
            select
...
create policy "only admin can read pan information for different employees" on management.personnel_action for
select
    using (
        exists (
            select
...
create policy "only the current user can read its own pan information" on management.personnel_action for
select
    using (
        employee_id = (
            select
...
create policy "only admin can update pan information for different employees" on management.personnel_action for
update using (
    exists (
        select
            1
